...
function check_arguments {
	...
	if [ "$1" -lt 5 ]; then
		echo "USAGE: "
		echo "$0 {process id} -cpu {utilization percentage} -mem {memory threshold (kB)} {maximum reports} {time interval}"
		exit
	fi
...
function generate_report {
	...
	num_files=$(ls -l ./reports_dir | wc -l)
	if [ "$num_files" -gt "$MAXIMUM_REPORTS" ] ; then
...
function calculate_cpu_usage {
	...
	#column 14 is utime and 15 is stime
...
function calculate_mem_usage
{
	...
	echo $(egrep "VmRSS" /proc/"$PID"/status) > tempout.txt
	mem_usage=$(awk '{print $2}' ./tempout.txt)
	rm tempout.txt
...
function notify
{
	...
	if [ $cpu_usage_int -gt $CPU_THRESHOLD -o $mem_usage -gt $MEM_THRESHOLD ] ; then
		mailx -s "Threshold exceeded" $USER < ./reports_dir/$(ls -t ./reports_dir | head -1)
	fi
